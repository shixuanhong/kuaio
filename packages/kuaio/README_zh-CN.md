...
- **⚡ 链式 API** - 直观流畅的方法链式调用
- **📝 字符串定义** - 简单的字符串快捷键定义
- **🔄 序列支持** - 只有按顺序依次按下定义的所有键或组合键后才会触发事件（如 `Ctrl+K, Ctrl+C`）
- **⌨️ 修饰键检测** - 完整支持修饰键，如 `Alt`、`Ctrl`、`Meta`、`Shift` 等
- **🎯 事件控制** - 支持事件控制功能，如 `preventDefault`、`stopPropagation` 等
- **🔧 灵活绑定** - 支持任何 EventTarget
...
| 浏览器      | 最低版本 | 基础功能    | 自动布局检查 | 状态    |
| ----------- | -------- | ----------- | ------------ | ------- |
| **Chrome**  | 69+      | ✅ 完全支持 | ✅ 完全支持  | 🟢 推荐 |
| **Edge**    | 79+      | ✅ 完全支持 | ✅ 完全支持  | 🟢 推荐 |
| **Firefox** | 51+      | ✅ 完全支持 | ❌ 不支持    | 🟡 兼容 |
| **Opera**   | 56+      | ✅ 完全支持 | ✅ 完全支持  | 🟢 推荐 |
| **Safari**  | 10.1+    | ✅ 完全支持 | ❌ 不支持    | 🟡 兼容 |

> **💡 使用建议**:
>
> - 🟢 推荐版本: 支持所有功能
> - 🟡 兼容版本: 支持基础功能，布局检查回退到默认 QWERTY 或手动注册并指定其他布局
...
Kuaio.createSync()
  .A()
  .on((event) => console.log('A 按下!', event))

// 物理键码
Kuaio.createSync()
  .KeyA()
  .on((event) => console.log('KeyA 按下!', event))

// 功能键
Kuaio.createSync()
  .F1()
  .on((event) => console.log('F1 按下!', event))

// 特殊键
Kuaio.createSync()
  .Enter()
  .on((event) => console.log('Enter 按下!', event))
Kuaio.createSync()
  .Escape()
  .on((event) => console.log('Escape 按下!', event))
Kuaio.createSync()
  .Backspace()
  .on((event) => console.log('Backspace 按下!', event))
```
